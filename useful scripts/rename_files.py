...
How it works: scans only the directory containing this script (not subfolders),
assigns each photo a unique random basename while preserving its extension,
and skips collisions with existing filenames or names already picked this run.
"""
...
# Files with these suffixes are treated as photos (case-insensitive via .suffix.lower())
...
def rename_photos(dry_run: bool = False) -> None:
    """Rename every photo file in this script's folder to a random unique name."""
    ...
    renamed, skipped = 0, 0
    assigned_names: set = set()  # names we've already planned this session (avoid duplicates)
